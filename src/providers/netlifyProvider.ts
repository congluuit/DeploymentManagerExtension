...
import { NetlifyClient, NetlifyBuildHook } from '../clients/netlifyClient';
import { NetlifyDeploy, ProjectInfo } from '../utils/types';
...
class NetlifyProviderAdapter implements ProviderAdapter {
    ...
    private async triggerBuild(client: NetlifyClient, siteId: string, siteName: string): Promise<void> {
        try {
    ...
    private async getOrCreateBuildHook(
        client: NetlifyClient,
        siteId: string,
        siteDeployHook: string | undefined,
        ...
        const created = await client.createSiteBuildHook(
            siteId,
            'Deployment Manager Redeploy Hook',
            branch
        );
        return { url: created.url };
    }

    private pickBestHook(hooks: NetlifyBuildHook[], branch: string | undefined): NetlifyBuildHook | null {
        if (hooks.length === 0) {
            return null;
        }
    ...
    private isSuccessState(state: string): boolean {
...
export const netlifyProvider: ProviderAdapter = new NetlifyProviderAdapter();
