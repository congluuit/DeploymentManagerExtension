...
class DashboardPanel {
    static createOrShow(extensionUri, runRefresh) {
        ...
        const panel = vscode.window.createWebviewPanel('deploymentManagerDashboardPanel', 'Deployment Manager Dashboard', activeColumn ?? vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async collectData() {
        ...
        if (coolifyConnected) {
            try {
                ...
                activity.push(...coolifyApps.slice(0, 12).map((app) => ({
                    id: `coolify-${app.uuid}`,
                    provider: 'Coolify',
                    projectId: app.uuid,
    ...
    buildVercelStatusDetail(deployment, hasLogs = false) {
        if (!deployment) {
            return undefined;
        }
        const parts = [];
    ...
    matchesWorkspaceOnVercel(projectInfo, projects) {
    ...
    matchesWorkspaceOnCoolify(projectInfo, apps) {
        const workspaceName = projectInfo.name.trim().toLowerCase();
        const workspaceRepo = this.normalizeRepoIdentifier(projectInfo.repoUrl);
    ...
    matchesWorkspaceOnNetlify(projectInfo, sites) {
        const workspaceName = projectInfo.name.trim().toLowerCase();
        const workspaceRepo = this.normalizeRepoIdentifier(projectInfo.repoUrl);
    ...
    getManagedProviderSectionHtml(provider, data) {
        ...
        const projectSection = data.projectInfo
            ? `
                <div class="project-grid">
                    ...
                    <div class="project-line"><span>Repository</span><strong>${DashboardPanel.escapeHtml(data.projectInfo.repoOwner && data.projectInfo.repoName
                ? `${data.projectInfo.repoOwner}/${data.projectInfo.repoName}`
                : 'Not detected')}</strong></div>
                    <div class="project-line"><span>Status</span><strong class="${existsRemotely ? 'status-ok' : 'status-idle'}">${existsRemotely ? `Deployed in ${deployedProviders.join(', ')}` : 'Not Deployed'}</strong></div>
                </div>
            `
